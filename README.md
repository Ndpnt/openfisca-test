...
These elements are described in different folders. All the modelling happens within the `openfisca_test` folder.
...
```sh
git clone git://github.com/Ndpnt/openfisca-test.git.git
cd openfisca-test.git
...
```sh
openfisca serve --port 5000 --country-package openfisca_test
```
...
curl -X POST -H "Content-Type: application/json" \
  -d @./openfisca_test/situation_examples/couple.json \
